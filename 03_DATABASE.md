...
**อัปเดตล่าสุด:** 2026-07-20 — รวม migration 002 (จ้างตรง) + 003 (โปรไฟล์) + 004 (แชท) + 005-007 + **008 (เวลาทำงาน / วันหมดอายุ / แจ้งเตือน)** แล้ว
...
> **โครงสร้าง DB ปัจจุบัน = schema.sql + 002 ถึง 008 รวมกัน**
> `migrate.js` รันทุกไฟล์เรียงตามชื่อ และข้าม error "มีอยู่แล้ว" → รันซ้ำได้ปลอดภัย
> ⚠️ **อย่าแก้ไฟล์ .sql เก่าที่รันไปแล้ว** — เขียนไฟล์ใหม่เป็นเลขถัดไป (`009_xxx.sql`)
...
| `start_date` / `end_date` | DATE | ช่วงของ **สัญญา** (วันเริ่ม–วันจบงาน) |
| 🆕 **`start_time` / `end_time`** | TIME NULL | ⭐ **รอบของแต่ละวัน** เช่น 08:00-17:00<br>แยกจาก `start_date/end_date` โดยตั้งใจ — ยัดรวมเป็น DATETIME แล้วจะตอบ "วันเสาร์เข้ากี่โมง" ไม่ได้เลย |
| `budget` | DECIMAL(10,2) | **งบที่ตั้งไว้** — ตกลงจริงในแชท |
| `budget_unit` | ENUM(`per_hour`,`per_day`,`per_month`,`total`) | |
| **`lat` / `lng`** | DECIMAL(10,7) **NULL ได้** | ⭐ พิกัดจริง — ถูก mask ก่อนส่งออก<br>🆕 **งานจ้างตรงปักหมุดได้แล้วตั้งแต่รอบ 008** (ก่อนหน้านี้เป็น NULL เสมอเพราะไม่มีหน้าจอให้ปัก) · ไม่ปักก็ยังส่งได้ |
...
| `cancel_reason` | VARCHAR(500) NULL | เหตุผลตอนยกเลิก **และตอนปฏิเสธคำขอจ้าง** (ใช้ร่วมกัน — งานหนึ่งจบได้ทางเดียว ทับกันไม่ได้) |
| `cancelled_by` | INT NULL | ใครเป็นคนกด |
| 🆕 **`status_changed_at`** | DATETIME NULL | ⭐ **สถานะเปลี่ยนล่าสุดเมื่อไหร่** — ใช้ตอบ "ถูกปฏิเสธมากี่ชั่วโมงแล้ว" (กฎซ่อน 24 ชม.)<br>`created_at` ตอบไม่ได้: คำขอที่ส่งเมื่อวานแล้วเพิ่งโดนปฏิเสธเมื่อกี้ ต้องยังโชว์อยู่ |
| 🆕 **`expires_at`** | DATETIME NULL | ⭐ **ประกาศงานหมดอายุเมื่อไหร่** (ตั้ง `NOW() + 14 วัน` ตอนโพส)<br>เก็บเป็นคอลัมน์จริง ไม่คำนวณสดจาก `created_at` → ต่ออายุรายตัวได้ + เปลี่ยนนโยบายแล้วงานเก่าไม่โดนย้อนหลังจนหายวับพร้อมกันทั้งกอง<br>**NULL = ไม่มีวันหมดอายุ** (งานเก่าก่อนรอบ 008) |
| `created_at` | TIMESTAMP | |

### วงจร status — แยกตาม hire_type

```
hire_type = 'open'  (ประกาศหาผู้ดูแล)
  open ──ผู้ว่าจ้างเลือกคน──→ matched ──กดงานเสร็จ──→ done
    └──ครบ 14 วัน──→ ยังเป็น open แต่หายจากหน้าค้นหา (expires_at ผ่านไปแล้ว)

hire_type = 'direct' (จ้างตรง)
  offered ──แคร์กิฟเวอร์กดรับ────→ matched ──กดงานเสร็จ──→ done
       └──แคร์กิฟเวอร์ปฏิเสธ──→ declined ──ผ่าน 24 ชม.──→ ซ่อนจากฝั่งผู้ว่าจ้าง

ทั้ง 2 แบบ: matched ──ยกเลิก (ต้องมีเหตุผล)──→ cancelled
```
...
> ⚠️ **"หมดอายุ" กับ "ซ่อนหลัง 24 ชม." ไม่ได้เปลี่ยน `status`** — เป็นการ **กรองตอน query** เท่านั้น
> ข้อมูลยังอยู่ครบ · เจ้าของประกาศยังเห็นงานที่หมดอายุ (มีป้าย "หมดอายุแล้ว") เพราะเขาต้องรู้ว่าทำไมไม่มีคนสมัคร

> **การค้นหาตามรัศมี** ใช้สูตร Haversine คำนวณสด ๆ ใน SQL (เฉพาะ `hire_type='open'` **และยังไม่หมดอายุ**)
...
## 7. 🆕 `notifications` — แจ้งเตือนในแอพ (กระดิ่งบนหัวเว็บ)
...
|---|---|---|
| `id` | INT PK | |
| `user_id` | INT FK | **คนที่ได้รับ** — ไม่ใช่คนที่ทำให้เกิดเหตุการณ์ |
| `job_id` | INT FK **NULL** | ⚠️ **`ON DELETE SET NULL` ไม่ใช่ CASCADE** |
| `type` | VARCHAR(40) | `job_cancelled` / `offer_declined` / `job_matched` / ... (ตัวกำหนดไอคอน) |
| `title` | VARCHAR(200) | บรรทัดหนา |
| `body` | VARCHAR(500) NULL | บรรทัดรายละเอียด |
| `read_at` | DATETIME NULL | NULL = ยังไม่อ่าน (= ตัวนับเลขแดง) |
| `created_at` | TIMESTAMP | |

> ⚠️ **ทำไม `job_id` ต้องเป็น `SET NULL`:** "ผู้ว่าจ้างถอนประกาศงานทิ้ง" (รอบ 007 **ลบแถวจริง**) คือเหตุการณ์ที่ผู้สมัครต้องรู้ที่สุด
> ถ้าเป็น CASCADE แจ้งเตือนจะถูกลบพร้อมงานในวินาทีเดียวกับที่มันควรถูกส่ง

**กติกา:** ไม่แจ้งเตือนคนที่กดเอง — เขาเพิ่งกดปุ่มนั้นกับมือ เห็น toast ไปแล้ว
**เขียนผ่าน `src/notify.js` เท่านั้น** — กลืน error เองทั้งหมด เพราะแจ้งเตือนล้มต้องไม่ทำให้การกระทำหลัก (ยกเลิกงาน) ล้มตาม

| type | เกิดเมื่อ | ใครได้รับ |
|---|---|---|
| `job_applied` | มีคนกดขอรับงาน | ผู้ว่าจ้าง |
| `application_withdrawn` | แคร์กิฟเวอร์ถอนคำขอ | ผู้ว่าจ้าง |
| `job_matched` | ผู้ว่าจ้างเลือกคนนี้ | แคร์กิฟเวอร์ที่ถูกเลือก |
| `job_not_chosen` | ผู้ว่าจ้างเลือกคนอื่น | ผู้สมัครที่เหลือ |
| `offer_received` | มีคำขอจ้างตรงส่งมา | แคร์กิฟเวอร์ |
| `offer_accepted` / `offer_declined` | แคร์กิฟเวอร์ตอบรับ/ปฏิเสธ | ผู้ว่าจ้าง |
| `job_withdrawn` / `offer_withdrawn` | ผู้ว่าจ้างถอนประกาศ/คำขอทิ้ง | ผู้สมัครทุกคน / แคร์กิฟเวอร์ที่ถูกยิงคำขอ |
| `job_cancelled` | ยกเลิกงานที่จับคู่แล้ว | อีกฝ่าย |
| `job_done` | ผู้ว่าจ้างกดปิดงาน | แคร์กิฟเวอร์ |
