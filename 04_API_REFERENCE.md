...
**อัปเดตล่าสุด:** 2026-07-20 — ตรงกับโค้ดจริงแล้ว (รวมรอบ 5: เวลาทำงาน / GPS จ้างตรง / หมดอายุ / แจ้งเตือน)
...
```json
{
  ...
  "start_time": "08:00", "end_time": "17:00",   // 🆕 รอบของแต่ละวัน (ไม่บังคับ)
...
```
→ `{ "ok": true, "id": 1, "lifetime_days": 14 }` — **บังคับ:** `title`, `budget`, `lat`, `lng`

> 🆕 **ประกาศงานมีอายุ 14 วัน** — `expires_at` ถูกตั้งอัตโนมัติตอนโพส
> ครบแล้วจะ **หายจากหน้าค้นหาของแคร์กิฟเวอร์ + กดขอรับไม่ได้** แต่ `status` ยังเป็น `open`
> เจ้าของยังเห็นใน `/mine` พร้อมธง `expired: true`
> เวลาที่ส่งมาไม่ตรงรูปแบบ `HH:MM` → เก็บเป็น `null` (ไม่ error) — ดู `readTime()` ใน `src/util.js`
...
```json
{
  "posted":  [ /* งานที่ฉันโพส (hire_type='open') + applicant_count + caregiver_name + 🆕 expired */ ],
  "applied": [ /* งานที่ฉันกดขอรับ + my_application_status */ ]
}
```
> 🆕 `expired: true` = ประกาศครบ 14 วันแล้ว (หายจากหน้าค้นหา แต่ยังโชว์ให้เจ้าของเห็นพร้อมป้าย "หมดอายุแล้ว")
...
{ "caregiver_id": 5, "title": "ดูแลคุณแม่ช่วงกลางวัน",
  ...
  "start_time": "09:00", "end_time": "18:00",   // 🆕 รอบของแต่ละวัน
  "elder_condition": "...", "tasks": "...",
  "address": "123/45 ซอยลาดพร้าว 15",  // เห็นเฉพาะแคร์กิฟเวอร์คนนี้เท่านั้น
  "lat": 13.7563, "lng": 100.5018,     // 🆕 ปักหมุดได้แล้ว (ไม่บังคับ)
  "area_label": "ปทุมวัน"
}
```
→ `{ "ok": true, "id": 6 }` — **บังคับ:** `caregiver_id`, `title`, `budget`

> 🆕 **`lat`/`lng` ปักได้แล้วตั้งแต่รอบ 008** (เดิมเขียนไว้ว่า "ไม่ต้องส่ง — งานจ้างตรงไม่ปักหมุด")
> ไม่ปักก็ยังส่งได้ แต่ **ถ้าส่งต้องมาครบทั้งคู่** มีด้านเดียวจะได้ `400`
...
| 🆕 `400` พิกัดไม่ครบ | ส่ง `lat` มาแต่ไม่ส่ง `lng` (หรือกลับกัน) |
...
{ "items": [{ "id": 6, "title": "...", "status": "offered",
  ...
  "decline_visible_hours": 24 }
```
> 🆕 **คำขอที่ถูกปฏิเสธจะหายจากลิสต์นี้เองหลัง 24 ชม.** (นับจาก `status_changed_at` ไม่ใช่ `created_at`)
> ซ่อน ไม่ใช่ลบ — ข้อมูลยังอยู่ใน DB ครบ

### `POST /:id/respond` — ตอบรับ / ปฏิเสธ (คนที่ถูกส่งคำขอไปหาเท่านั้น)
```json
{ "decision": "accept" }                                  // หรือ
{ "decision": "decline", "reason": "ติดงานประจำช่วงนั้น" }   // 🆕 เหตุผล (ไม่บังคับ)
```
→ `{ "ok": true, "status": "matched" }` | `{ "ok": true, "status": "declined" }`

> 🆕 `reason` เก็บลง `jobs.cancel_reason` (คอลัมน์เดียวกับตอนยกเลิกงาน — งานหนึ่งจบได้ทางเดียว ทับกันไม่ได้)
> ผู้ว่าจ้างเห็นเหตุผลนี้ทั้งในแจ้งเตือนและในแผ่นรายละเอียด
...
{ "items": [{ "job_id": 1, "title": "...", "status": "open", "hire_type": "open",
              ...
              "unread": 2,
              "sort_at": "...",                     // 🆕 ตัวเรียง = ข้อความล่าสุด หรือเวลาที่ห้องเกิด
              "other_online": true,                 // จุดเขียว — มาจากทะเบียน socket ไม่ใช่ DB
              "other_last_seen": "2026-07-16T20:52:42.000Z" }] }  // ไว้โชว์ "เห็นล่าสุดเมื่อ..."
```

> 🆕 **ซ่อนห้องของงานที่ปิดแล้ว** — กรอง `jobs.status <> 'done'`
> ยกเลิก/ปฏิเสธ **ไม่ซ่อน** เพราะ 2 กรณีนั้นมักมีเรื่องต้องเคลียร์กันต่อ · ข้อความเก่ายังอยู่ใน DB ครบ

> 🆕 **เรียง "แชทใหม่อยู่บน"** — `ORDER BY sort_at DESC` โดย `sort_at = COALESCE(ข้อความล่าสุด, เวลาที่ห้องเกิด)`
> ของเดิม `ORDER BY last_at IS NULL, last_at DESC` ดันห้องที่ยังไม่มีใครทักลงล่างสุดเสมอ
...
### `GET /?role=` — ตัวเลขแดงบนแท็บ + กระดิ่ง (หน้าเว็บเรียกทุก 15 วินาที)
```json
{ "chat": 3, "applicants": 1, "offers": 2, "alerts": 4 }
```

| ค่า | นับอะไร | ไปโผล่ที่ |
|---|---|---|
| `chat` | ข้อความที่ส่งมาหาฉันและยังไม่อ่าน (`read_at IS NULL`) **ของงานที่ยังไม่จบ** | แท็บแชท |
| `applicants` | คนที่มากดขอรับงานที่ฉันโพส และฉันยังไม่ได้เลือกใคร | แท็บภารกิจของฉัน (ฝั่งผู้จ้าง) |
| `offers` | คำขอจ้างตรงที่ส่งมาหาฉัน ยังไม่ได้ตอบ | แท็บภารกิจของฉัน (ฝั่งแคร์กิฟเวอร์) |
| 🆕 `alerts` | แจ้งเตือนที่ยังไม่ได้เปิดอ่าน | **กระดิ่งบนหัวเว็บ** |

> ⚠️ `chat` ไม่นับห้องของงานที่ `status='done'` — ห้องพวกนั้นถูกซ่อนไปแล้ว (ดู `/api/chat/threads`)
> ถ้ายังนับ เลขแดงจะค้างโดยที่ผู้ใช้กดเข้าไปอ่านไม่ได้เลยสักทาง

### 🆕 `GET /list` — รายการในกระดิ่ง (ล่าสุด 40 รายการ)
```json
{ "items": [{ "id": 12, "job_id": 6, "type": "job_cancelled",
              "title": "งานถูกยกเลิก",
              "body": "\"ดูแลคุณแม่\" ถูกยกเลิกโดยผู้ว่าจ้าง — เหตุผล: ...",
              "created_at": "...", "read_at": null,
              "job_status": "cancelled", "hire_type": "direct" }] }
```
> `job_id` เป็น `null` ได้ ถ้างานถูกลบทิ้ง (ผู้ว่าจ้างถอนประกาศ) — แจ้งเตือนยังอยู่ ดู `03_DATABASE.md` ข้อ 7

### 🆕 `POST /read` — เปิดกระดิ่ง = อ่านหมดแล้ว
→ `{ "ok": true }` · ไม่มีการอ่านทีละอัน (เปิดแผ่นขึ้นมาก็เห็นทุกอันพร้อมกันอยู่แล้ว)

### 🆕 Socket event `notify`
เด้งสดให้คนที่เปิดเว็บค้างอยู่ ไม่ต้องรอรอบ poll 15 วิ — payload หน้าตาเหมือน 1 item ใน `/list`
